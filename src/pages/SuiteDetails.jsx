    ...
    <main id="main" className="scroll-mt-24 pb-24 pt-28 text-ink">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <header className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6 mb-10">
          <div>
            <p className="text-accent eyebrow mb-3">
            ...
            {count > 0 && (
              <p className="flex items-center gap-2 mt-4 text-sm text-ink-soft">
                <Star className="w-4 h-4 fill-accent text-accent" />
                <span className="text-ink">{average}</span>
                <span className="text-ink-faint">·</span>
                <a href="#suite-reviews" className="hover:text-accent transition-colors">
          ...
          <div className="shrink-0 lg:text-right">
            <p className="font-sans tabular-nums text-3xl text-ink">
              {currency(suite.price)}
              <span className="text-sm text-ink-soft font-sans"> / night</span>
            </p>
            <Link
        ...
        <Plate
          ...
          alt={suite.name}
          loading="eager"
          className="w-full h-[58vh] min-h-80 object-cover rounded-card border border-line shadow-2xl mb-6"
        />

        {/* ---- Specs strip --------------------------------------------- */}
        <dl className="grid grid-cols-2 lg:grid-cols-4 gap-px bg-panel rounded-card overflow-hidden mb-20">
          {specs.map(({ icon: Icon, label, value }) => (
            <div key={label} className="bg-panel p-5">
              <dt className="flex items-center gap-2 text-[0.7rem] uppercase tracking-widest text-accent mb-2">
              ...
              >
                <div className="w-full lg:w-3/5">
                  <Plate
                    src={room.image}
                    alt={room.name}
                    className="w-full h-80 lg:h-[28rem] object-cover rounded-card border border-line"
                  />
                </div>
                <div className="w-full lg:w-2/5 space-y-5">
                  <h3 className="text-balance text-2xl font-display font-light">{room.name}</h3>
                  <p className="text-pretty text-ink-soft font-light leading-relaxed">{room.copy}</p>
        ...
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-20">
          <div className="rounded-card border border-line bg-panel p-8">
            <h2 className="text-balance flex items-center gap-3 text-2xl font-display font-light mb-6">
              <Sofa className="w-5 h-5 text-accent" /> What is in the rooms
            </h2>
            <ul className="space-y-3">
              {suite.furnishings.map((item) => (
                <li key={item} className="flex items-start gap-3 text-ink-soft font-light">
                  <span aria-hidden className="mt-2 w-1 h-1 rounded-full bg-accent-fill shrink-0" />
                  {item}
                </li>
              ))}
            </ul>
          ...
          <div className="rounded-card border border-line bg-panel p-8">
            <h2 className="text-balance flex items-center gap-3 text-2xl font-display font-light mb-4">
              <Droplets className="w-5 h-5 text-accent" /> The bathrooms
            </h2>
            <p className="text-pretty text-ink-soft font-light leading-relaxed mb-6">{suite.bath.copy}</p>
            <ul className="space-y-3">
            ...
            {suite.gallery.map((shot, idx) => (
              ...
              >
                <Plate
                  src={shot.src}
                  alt={shot.alt}
                  className="w-full h-auto object-cover rounded-card border border-line group-hover:border-accent group-focus-visible:ring-2 group-focus-visible:ring-accent transition-colors"
                />
              </button>
            ))}
          </div>
        ...
        {lightbox && createPortal(
          ...
          >
            <button 
              onClick={() => setLightbox(null)}
              className="absolute top-6 right-6 p-2 text-ink-soft hover:text-ink transition-colors z-10"
          ...
          <section id="suite-reviews" className="scroll-mt-24 mb-20">
            <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-8">
              ...
              <p className="flex items-center gap-2 text-sm text-ink-soft">
                ...
                <span className="text-ink">{average}</span> from {count} verified{' '}
                {count === 1 ? 'stay' : 'stays'}
              </p>
            </div>
            ...
            <div className="reveal-group grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {suite.reviews.map((review) => (
                <blockquote
                  key={review.name}
                  className="rounded-card border border-line bg-panel p-7 flex flex-col"
                >
                  ...
                  <p className="text-pretty [hanging-punctuation:first] text-ink-soft font-light leading-relaxed italic flex-grow">
                    “{review.text}”
                  </p>
                  <footer className="mt-6 pt-5 border-t border-line">
                    <p className="text-ink text-sm">{review.name}</p>
          ...
          <div>
            ...
            <ul className="space-y-3">
              {standingPrivileges.map((privilege) => (
                <li key={privilege.id} className="flex items-start gap-3 text-ink-soft font-light">
            ...
            <p className="text-sm text-ink-faint font-light mt-5">
        ...
        <div className="rounded-card border border-accent/30 bg-accent-fill/[0.07] p-8 flex flex-col sm:flex-row sm:items-center justify-between gap-6">
          <div>
