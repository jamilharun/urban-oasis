...
export default function Testimonials() {
  return (
    <section className="py-28 bg-surface-sunk text-ink relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="reveal text-center mb-16 max-w-measure mx-auto">
          <p className="text-accent eyebrow mb-4">Word of Mouth</p>
        ...
        <div className="reveal-group grid grid-cols-1 md:grid-cols-3 gap-8">
          {reviews.map((review) => (
            <div key={review.id} className="bg-panel p-8 rounded-card shadow-sm border border-line flex flex-col justify-between transition-shadow duration-300 hover:shadow-md">
              <div>
                ...
                <p className="text-pretty [hanging-punctuation:first] text-ink-soft font-light leading-relaxed mb-8 italic text-lg">
                  "{review.text}"
                </p>
              </div>
              <div>
                <h3 className="text-balance font-medium text-lg font-display">{review.name}</h3>
                <p className="text-xs text-ink-soft uppercase tracking-widest mt-1">{review.source}</p>
              </div>
            </div>
          ))}
        </div>
